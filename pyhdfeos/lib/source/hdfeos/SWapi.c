...
Sept. 14, 2004 Melissa Porricelli
                            NCR 39839 Added SWindexinfo function to obtain 
                              indices of subsetted swath
Nov 15, 2005   Tom Roegner  NCR 44092 - Added CYGWIN capability
...
#define NSWATHREGN 512
#define MAXNREGIONS 2048
struct swathRegion
{
    int32 fid;
    int32 swathID;
...
intn SWscan2longlat(int32, char *, VOIDP, int32 [], int32 [], int32 *,
                    int32, int32);
int32 SWidxinfo(int32, char *, int32, char *, int32 *);
int32 SWllmapedidxinfo(int32 swathID, char *object, int32 *index0, int32 *index1);
intn SWtileinfo(int32, char *, int32 *, int32 *,
		int32 []);
/*----------------------------------------------------------------------------|
|  BEGIN_PROLOG                                                               |
|                                                                             |
...
SWcreate(int32 fid, char *swathname)
{
    ...
    char            utlbuf[UTLSTR_MAX_SIZE];/* Utility buffer */
	...
	HEreport("Cannot call SWcreate on a file opened read-only.\n");
      }
    
    /* Check swathname for length */
    /* -------------------------- */
    else if ((intn) strlen(swathname) > VGNAMELENMAX)
    {
	status = -1;
	HEpush(DFE_GENAPP, "SWcreate", __FILE__, __LINE__);
...

{
	...
	if (metaptrs[0] < metaptrs[1] && metaptrs[0] != NULL)
	{
	    ...
	    {
	      int32 strlen_utlstr;
	      /* If the value in utlstr has leading and trailing quotes, get rid 
		 of them before applying atol, otherwise atol will return zero */
	      
	      strlen_utlstr = strlen(utlstr);
	      if(utlstr[0] == '"' && utlstr[strlen_utlstr-1] == '"')
		{
		  memmove(utlstr, utlstr + 1, strlen(utlstr) - 2);
		  utlstr[strlen(utlstr) - 2] = 0;
		  size = (int32)atol(utlstr);
		}
	      else
		{
		  size = (int32)atol(utlstr);
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: SWdefdimscale                                                    |
|                                                                             |
|  DESCRIPTION: Defines a dimension scale for all fields dimension of a swath |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
|         and sets dimension scale for that field calling SWsetdimscale()     |
...
-----------------------------------------------------------------------------*/
intn
SWdefdimscale(int32 swathID, char *dimname, int32 dimsize, 
	      int32 numbertype, VOIDP data)
{
     ...
     */
    status = SWchkswid(swathID, "SWdefdimscale",
    ...
    if (status == 0)
    {
      ...
      if ( dimsize == 0 )
	{
	    HEpush(DFE_GENAPP, "SWdefdimscale", __FILE__, __LINE__);
      ...
      */
      ...
      /* Remove offset from swath ID */
      gID = swathID % idOffset;
      
      /* Loop through all SDSs in swath */
      /* ------------------------------ */
      if(SWXSwath[gID].nSDS > 0)
	{
	  for (i = 0; i < SWXSwath[gID].nSDS; i++)
	    {
	      /* If active SDS ... */
	      /* ----------------- */
	      if (SWXSwath[gID].sdsID[i] != 0)
		{
		  /* Get SDS ID, name, rankSDS, and dimensions */
		  /* ----------------------------------------- */
		  sdid = SWXSwath[gID].sdsID[i];
		  ...
		  status = SWfieldinfo(swathID, fieldname, &rank, dims, &nt, dimlist);
		  if (status != 0)
		    {
		      status = -1;
		      HEpush(DFE_GENAPP, "SWdefdimscale", __FILE__, __LINE__);
		  ...
		  if (Dimindex == -1)
		    ...
		    {
		      foundField = 1;
		      status = SWsetdimscale(swathID, fieldname, dimname, dimsize, 
					     numbertype, data);
		      if ( status != 0 )
			{
			  status = -1;
      ...
      if(i == SWXSwath[gID].nSDS && foundField == 0) /* did not find any fields 
							with the dimname       */
	{
	  HEpush(DFE_GENAPP, "SWdefdimscale", __FILE__, __LINE__);
	  HEreport("Did not find any hdfeos field in the swath \n");
	  status = -1;
	  goto error;
	}
      else
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: SWsetdimscale                                                    |
|                                                                             |
|  DESCRIPTION: Defines a dimension scale for a fields dimension of a swath   |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
-----------------------------------------------------------------------------*/
intn
SWsetdimscale(int32 swathID, char *fieldname, char *dimname,int32 dimsize, 
	      int32 numbertype, VOIDP data)
{
    intn            i;		/* Loop index */
    intn            status;	/* routine return status variable */
     ...
     * and swath root Vgroup ID
     */
    status = SWchkswid(swathID, "SWsetdimscale",
		       &fid, &sdInterfaceID, &swVgrpID);
	...
	/* ----------------------- */
	status = SWfieldinfo(swathID, fieldname, &rankSDS, dims, &dum, NULL);

	if (status != 0)
	{
	    HEpush(DFE_GENAPP, "SWsetdimscale", __FILE__, __LINE__);
	...
	gID = swathID % idOffset;
	Vgetname(SWXSwath[gID].IDTable, swathname);
	...
	if(i == SWXSwath[gID].nSDS) /* did not find fieldname */
	  {
	    HEpush(DFE_GENAPP, "SWsetdimscale", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
...
-----------------------------------------------------------------------------*/
intn
SWdefdimstrs(int32 swathID, char *dimname, char *label, char *unit, char *format)
{
    intn            i;		/* Loop index */
    intn            status;	/* routine return status variable */
     ...
     * and swath root Vgroup ID
     */
    status = SWchkswid(swathID, "SWdefdimstrs",
		       &fid, &sdInterfaceID, &swVgrpID);
	  ...
	  for (i = 0; i < SWXSwath[gID].nSDS; i++)
	      ...
	      if (SWXSwath[gID].sdsID[i] != 0)
		{
		  ...
		  if (status != 0)
		    {
		      status = -1;
		      HEpush(DFE_GENAPP, "SWdefdimstrs", __FILE__, __LINE__);
		      HEreport("Field: \"%s\" not found.\n", fieldname);
		      goto error;
		    }
		  ...
		  if (Dimindex == -1)
		    ...
		    {
		      foundField = 1;
		      SWsetdimstrs(swathID, fieldname, dimname, label, unit, format);
		      if ( status != 0 )
			{
			  status = -1;
			  HEpush(DFE_GENAPP, "SWdefdimstrs", __FILE__, __LINE__);
	...
	{
	  HEpush(DFE_GENAPP, "SWdefdimstrs", __FILE__, __LINE__);
	  HEreport("Did not find any hdfeos field in the swath with the dimension specified\n");
	  status = -1;
	  goto error;
	}
      else
...
-----------------------------------------------------------------------------*/
intn
SWsetdimstrs(int32 swathID, char *fieldname, 
	     char *dimname, char *label, char *unit, char *format)
{
     ...
     * and swath root Vgroup ID
     */
    status = SWchkswid(swathID, "SWsetdimstrs",
		       &fid, &sdInterfaceID, &swVgrpID);
	...
	if (status != 0)
	{
	    HEpush(DFE_GENAPP, "SWsetdimstrs", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
	    goto error;
	}
...
-----------------------------------------------------------------------------*/
intn
SWgetdimscale(int32 swathID, char *fieldname, char *dimname, int32 *dimsize, 
	      int32 *data_type, VOIDP data)
{
     ...
     * and swath root Vgroup ID
     */
    status = SWchkswid(swathID, "SWgetdimscale",
		       &fid, &sdInterfaceID, &swVgrpID);
	...
	if (status != 0)
	{
	    HEpush(DFE_GENAPP, "SWgetdimscale", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
	    return ( status );
	}
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: SWgetdimstrs                                                     |
|                                                                             |
|  DESCRIPTION: Get dimension strs for a dimension of a field in a swath      |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
-----------------------------------------------------------------------------*/
intn
SWgetdimstrs(int32 swathID, char *fieldname, 
	     char *dimname, char *label, char *unit, char *format, intn len)
{
     ...
     * and swath root Vgroup ID
     */
    status = SWchkswid(swathID, "SWgetdimstrs",
		       &fid, &sdInterfaceID, &swVgrpID);
	...
	if (status != 0)
	{
	    HEpush(DFE_GENAPP, "SWgetdimstrs", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
	    return ( status );
	}
	...
	if(i == SWXSwath[gID].nSDS) /* did not find fieldname */
	  {
	    HEpush(DFE_GENAPP, "SWgetdimsctrs", __FILE__, __LINE__);
	    HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	    status = -1;
	    return ( status );
	  }
...

{
	...
	*/
	...
	{
	  /* Tried to add this section to get correct dim sizes 
	     from the field itself.Though it seems to work by itself, 
	     but when called in other functions some othetr functions, 
	     such as SWwrrdfield() in SWwrfld() for a 1D field causes problem.
	     NCR 8048515 was fixed (grid problem) in GDapi.c . 
	     The better treatment of fieldinfo for swath will be fixed later
	     if needed under a separate NCR

	  int swathID_IsSubset = 0;
	  for(rgid=0; rgid < NSWATHREGN; rgid++)
	    {
	      if(SWXRegion[rgid] == 0)
		{
		  continue;
		...
		{
		  if(SWXRegion[rgid]->swathID == swathID)
		    {
		      swathID_IsSubset = 1;
		      break;
		    }
	  ...
	  if (swathID_IsSubset == 0)
	      ...
	      if (*rank == 1)
		{
		  ...
		  status2 = SW1dfldsrch(fid, swathID, fieldname, "r",
				       &dum, &vdataID, &dum);
		  
		  actualsize = VSelts(vdataID);
		  if (dims[0] < actualsize)
		    {
		      dims[0] = actualsize;
		    }
	      ...
	      else
		{
		  ...
		  status2 = SWSDfldsrch(swathID, sdInterfaceID, fieldname,
				       &dum, &dum, &dum, &dum, actualdims,
				       &dum);
		  if (status2 == 0)
		    {
		      for (i = 0; i < ndims; i++)
	  ...
	  if(can_encode == (COMP_DECODER_ENABLED|COMP_ENCODER_ENABLED))
		{
		  if( compparm[0] != 2 && compparm[0] != 4 && 
		    ...
		    {	  
		      status = -1;
		      HEpush(DFE_GENAPP, "SWdefcomp", __FILE__, __LINE__);
...

{
    ...
    char            utlbuf[UTLSTR_MAX_SIZE];/* Utility buffer */
    char            utlbuf2[UTLSTR_MAX_SIZE];	/* Utility buffer 2 */
    char           *ptr[32];	/* String pointer array */
    char            swathname[80];	/* Swath name */
			...
			if(can_encode == (COMP_DECODER_ENABLED|COMP_ENCODER_ENABLED))
			  {
			    ...
			    c_info.szip.pixels_per_block = SWXSwath[sID].compparm[0];
			    if( SWXSwath[sID].compparm[1] == SZ_EC)
			      {
				c_info.szip.options_mask = SZ_EC_OPTION_MASK | SZ_RAW_OPTION_MASK;
			      }
			    else if ( SWXSwath[sID].compparm[1] == SZ_NN)
			      {
...

{
	...
	if (status != -1)
	{
		...
		if ( status != 0 )
		  {
		    status1 = SWcompinfo(swathID, fieldname, &compcode, NULL);
	...
	else
	{
		...
		if (strcmp(code, "w") == 0)
		{
			...
			nrec = VSread(vdataID, buf, 1 + (count[0] - 1) * incr[0],
				      FULL_INTERLACE);
		    }
		    else
		    {
		...
		else
		{
		    ...
		    (void) VSread(vdataID, buf, 1 + (count[0]- 1) * incr[0],
			   FULL_INTERLACE);
...

{
	...
	if(start == NULL)
	{ 
	    HEpush(DFE_NOSPACE,"SWrdfld", __FILE__, __LINE__);
	    return(-1);
	}
	stride = (int32 *) malloc(4 * rank);
	if(stride == NULL)
	       ...
	       int32 mode)
{
	...
	if (status == 0)
	{
	     ...
	     * FLOAT64
	     */
	    if (nt == DFNT_FLOAT32 || nt == DFNT_FLOAT)
	    {
		for (i = nElem - 1; i >= 0; i--)
		{
		...
		for (i = 0; i < edge[0]; i++)    
		{
			...
			if (status != 0)
			  {
			    HEpush(DFE_GENAPP, "SWdefboxregion", __FILE__, __LINE__);
			    HEreport("Failed reading longitude field.\n");
			    free(flag);
			    free(lonArr);
			    free(latArr);
			    return (-1);
			...
			if (status != 0)
			  {
			    HEpush(DFE_GENAPP, "SWdefboxregion", __FILE__, __LINE__);
			    HEreport("Failed reading latilude field.\n");
			    free(flag);
			    free(lonArr);
			    free(latArr);
			    return (-1);
		...
		int32 mode)
{
    ...
    intn            statusRead;	/* Status from SWreadfield for time */
	...
	if (statTime  == 0)
	{
		...
		/* Read "Time" field */
		/* ----------------- */
		statusRead = SWreadfield(swathID, "Time",
    ...
    *size = -1;
	...
	if (SWXRegion[regionID]->nRegions > 0 || SWXRegion[regionID]->scanflag == 1)
	{
		...
		for (k = 0; k < SWXRegion[regionID]->nRegions; k++)
		{
				...
				HEpush(DFE_GENAPP, "SWregioninfo",
				       __FILE__, __LINE__);
				HEreport("Cannot get field info for \"%s\".\n","scan_no");
				  return(status);
			      }
			 }
                         if(SWXRegion[regionID]->band8flag == 2 ||
   ...
   /* ================================================= */
	...
	HEreport("Field: \"%s\" not found.\n", tfieldname);
	  ...
	  if(status == 0) {
	    ...
	    if(buffer == NULL)
	      { 
		HEpush(DFE_NOSPACE,"SWdefscanregion", __FILE__, __LINE__);
		if (dfieldlist != NULL)
		  free(dfieldlist);
		free(tfieldname);
		return(-1);
...
|                         than utlstr                                         |
|  Nov 96   Joel Gales    Call SWchkswid to check for proper swath ID         |
...

{
	...
	else
	{
		...
		if(indvdims == NULL)
	      ...
	      {
	        HEpush(DFE_GENAPP,"SWdetach", __FILE__, __LINE__);
		return(-1);	
	      }
...
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
|  status         int32               return status (0) SUCCEED, (-1) FAIL    |
|                                                                             |
|  INPUTS:                                                                    |
|  regionID       int32               Region ID                               |
...
|  rank           int32               field rank                              |
|  dimlist        char*               field dimension list                    |
|  indices        int32               indices of region for along track dim.  |
|                                                                             |
|  NOTES:                                                                     |
|                                                                             |
|   Date     Programmer   Description                                         |
|  ======   ============  =================================================   |
|  Aug 04   M. Porricelli Original development, derived from HDFEOS5          |
...
-----------------------------------------------------------------------------*/
int32
SWindexinfo(int32 regionID, char *object, int32 *rank, char *dimlist, int32 *indices[])
{

  int32           status = -1;   /* routine return status variable */
  intn            j,k, fi, gi;   /* loop index                     */

  int32           nt;            /* number types                   */
  int32           dims[8];       /* Dimension array                */
  char            tempdimlist[HDFE_DIMBUFSIZE];/* Dimension list   */
  ...
  if (regionID < 0 || regionID >= NSWATHREGN)
  {
      ...
      HEpush(DFE_GENAPP, "SWindexinfo", __FILE__, __LINE__);
      HEreport(errbuf, __FILE__, __LINE__);
      return(status);
  }
  /* regionID is reasonable number, continue ...*/
  /* Check for active region ID */
  /* -------------------------- */
  if (SWXRegion[regionID] == 0)
  {
  ...
  /* ------------------------------- */
  status = SWfieldinfo(SWXRegion[regionID]->swathID, object, rank, dims, &nt, tempdimlist);
  if (status != 0)
  {
   ...
   * indicies.
   */
  status = SWllmapedidxinfo(SWXRegion[regionID]->swathID,object,&index0,&index1);
  if (status != 0)
  {
      sprintf(errbuf, "Field \"%s\" not found.\n", object);
      HEpush(DFE_GENAPP, "SWindexinfo", __FILE__, __LINE__);
  ...
  for (j = 0; j < *rank; j++)
  {
	...
	if (j <index0)
      ...
      if (SWXRegion[regionID]->StartVertical[gi] == -1)
      {
          indices[fi][0] = SWXRegion[regionID]->StartRegion[gi];
      }
      else
      {
          indices[fi][0] = SWXRegion[regionID]->StartVertical[gi];
      }


      if (SWXRegion[regionID]->StopVertical[gi] == -1)
      {
          if (SWXRegion[regionID]->StopRegion[gi] == 0)
          {
              indices[fi][1] = dims[fi] - 1;
          }
          else
          {
              indices[fi][1] = SWXRegion[regionID]->StopRegion[gi];
          }
      }
      else
      {
          if (SWXRegion[regionID]->StopVertical[gi] == 0)
          {
              indices[fi][1] = dims[fi] - 1;
          }
          else
          {
              indices[fi][1] = SWXRegion[regionID]->StopVertical[gi];
          }
      }
  }
...
|                                                                             |
|  INPUTS:                                                                    |
|  swathID        int32               Swath ID                                |
|  object         char*               field name                              |
|                                                                             |
|                                                                             |
|  OUTPUTS:                                                                   |
|  indices        int32               0th and 1st indices of the field        |
|                                                                             |
|  NOTES:                                                                     |
|                                                                             |
|   Date     Programmer   Description                                         |
...
-----------------------------------------------------------------------------*/
int32
SWllmapedidxinfo(int32 swathID, char *object, int32 *index0, int32 *index1)
{
  int32           status = -1;   /* routine return status variable */
  intn            statMap = -1;  /* Status from SWmapinfo          */
  char            dimlist[HDFE_DIMBUFSIZE];/* Dimension list       */
  intn            i,j,k, iDim;   /* loop index                     */
  char            errbuf[256];   /* Error buffer                   */
  int32           nt, ntype;     /* number types                   */
  int32           dims[8];       /* Dimension array                */
  int32           rank;
  ...
  /* Get data field info */
  /* ------------------- */
  status = SWfieldinfo(swathID, object, &rank,
		       dims, &ntype, dimlist);
  if (status != 0)
    {
      status = -1;
      HEpush(DFE_GENAPP, "SWllmapedidxinfo", __FILE__, __LINE__);
      HEreport("Field \"%s\" Not Found.\n", object);
      return(-1);
    }

  status = SWfieldinfo(swathID, "Longitude", &rank, 
		       dumdims, &nt, geodim);
  if (status != 0)
  {
      sprintf(errbuf, "Field \"%s\" not found.\n", "Longitude");
  ...
  for(iDim=0; iDim<8; iDim++)
      ...
      for (i = 0; i < nDim; i++)
	{
	  ...
	  statMap = SWmapinfo(swathID, geodimbuf, utlbuf,
			      &dum, &incr);
      ...
      if (found == 0)
	{
	  index[iDim] = EHstrwithin(geodimbuf, dimlist, ',');
      ...
      if (found == 0)
	{
	  /* Get size of geo dim & allocate space of index mapping */
	  /* ----------------------------------------------------- */
	  dum = SWdiminfo(swathID, geodimbuf);
	  idxmap = (int32 *) calloc(dum, sizeof(int32));
	  if(idxmap == NULL)
	    { 
	      HEpush(DFE_NOSPACE,"SWllmapedidxinfo", __FILE__, __LINE__);
	      return(-1);
	    }
	  ...
	  for (i = 0; i < nDim; i++)
	    {
	      ...
	      idxMapElem = SWidxmapinfo(swathID, geodimbuf, utlbuf, idxmap);
...
|  FUNCTION: SWidxinfo                                                        |
|                                                                             |
|  DESCRIPTION: FORTRAN wrapper around SWindxinfo                              |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
|  fortindices    int32*              indices of region for along track dim.  |
|                                                                             |
|  NOTES:                                                                     |
|                                                                             |
|   Date     Programmer   Description                                         |
...
-----------------------------------------------------------------------------*/
int32
SWidxinfo(int32 regionID, char *object, int32 rank, char *fortdimlist, int32 *fortindices)
{
  int32     i, j;                        /* Loop index           */

  int32     status     = -1; /* routine return status variable */
  char      errbuf[256];     /* Error message buffer */
  int32     *indices[8];
  int32     swap       = 0;              /* Temporary dimension size   */
  ...
  /* Allocate space for C order dimension list */
  /* ----------------------------------------- */
  dimlist = (char *)calloc(1024, sizeof(char));    /* in hdfeos5 code, HE5_HDFE_UTLBUFSIZE=1024 */
  if(dimlist == NULL)
  {
          sprintf(errbuf,"Cannot allocate memory for input list.\n");
          HEpush(DFE_NOSPACE, "SWidxinfo", __FILE__, __LINE__);
          HEreport(errbuf, __FILE__, __LINE__);
          return(-1);
  }
  ...
  for (i = 0; i < 8; i++)
  {
          indices[i] = (int32 *)malloc(2*sizeof(int32));
  }

  /* Call SWidxinfo */
  /* ---------------- */
  status = SWindexinfo(regionID, object, &rank, dimlist, indices);
  if (status == -1)
  {
          sprintf(errbuf,"Cannot get information about indices.\n");
          HEpush(DFE_GENAPP, "SWidxinfo", __FILE__, __LINE__);
          HEreport(errbuf, __FILE__, __LINE__);
          free(dimlist);
          for (i = 0; i < 8; i++)
          {
  ...
  {
      status = EHrevflds(dimlist, fortdimlist);
      if(status == -1)
      {
  ...
  for (j = 0; j < rank; j++)
  {
      for (i = 0; i < 2; i++)
      {
          fortindices[j*2+i] = (int32)indices[j][i];
      }
  }
...
/***********************************************
SWchunkinfo --
     This function queries the field to determine if it is chunked.  If it is
     chunked, one can retrieve some of the characteristics of the chunks.
...
intn
SWchunkinfo(int32 swathID, char *fieldname, int32 * chunkcode, int32 * chunkrank,
	   int32 chunkdims[])

{
    ...
    int32           dum;	/* Dummy variable */
    int32           rankSDS;	/* Rank of SDS/Field/chunk */

    int32           dims[8];	/* Field/SDS dimensions */
    int32           chunkFlags;	/* flag to determine if field is chunkd */
    HDF_CHUNK_DEF   chunkDef;	/* union holding tiling info. */


    /* Check if improper swathID */
    status = SWchkswid(swathID, "SWchunkinfo", &fid, &sdInterfaceID, &dum);
    if (status == 0)
    {
	...
	if (status == 0)
	{
	    ...
	    if (status != 0) {
	      HEpush(DFE_GENAPP, "SWchunkinfo", __FILE__, __LINE__);
	    ...
	    status = SDgetchunkinfo(sdid, &chunkDef, &chunkFlags);

	    /* If field is unchunked, return unchunkd flag */
	    if (chunkFlags == HDF_NONE)
	    {
		*chunkcode = HDFE_NOTILE;
		return (status);
	    }

	    /* IF field is chunkd or chunkd with compression */
	    else if ((chunkFlags == HDF_CHUNK) ||
		     (chunkFlags == (HDF_CHUNK | HDF_COMP)))
	    {
		if (chunkcode != NULL)
		{
		    *chunkcode = HDFE_TILE;
		...
		if (chunkdims != NULL)
		{
		    /* Assign size of chunk dimensions */
		    for (i = 0; i < rankSDS; i++)
		    {
			chunkdims[i] = chunkDef.chunk_lengths[i];
		    }
		}
	    }
	}
...
This function gets information on the chunking of a field (FORTRAN wrapper around
SWchunkinfo)
...
intn
SWchkinfo(int32 swathID, char *fieldname, int32 * chunkcode, int32 * chunkrank,
	  int32 fortchunkdims[])
{
    ...
    int32           ntype;	/* Field numbertype */
    int32           dims[8];	/* Field dimensions */
    int32          *chunkdims;	/* Pointer dimension size array (C order) */
    ...
    if (status == 0)
    {
	chunkdims = malloc(rank * sizeof(int32));
	if(chunkdims == NULL)
	{ 
	    HEpush(DFE_NOSPACE,"SWchkinfo", __FILE__, __LINE__);
	    return(-1);
	}
	...
	for (i = 0; i < rank; i++)
	{
	    chunkdims[i] = fortchunkdims[rank - 1 - i];
	}

	/* call SWchunkinfo with C ordered array */
	status = SWchunkinfo(swathID, fieldname, chunkcode, chunkrank, chunkdims);

	free(chunkdims);
    }
    else
    {
	HEpush(DFE_GENAPP, "SWchkinfo", __FILE__, __LINE__);
	HEreport("Fieldname \"%s\" does not exist.\n", fieldname);
	status = -1;
...
FCALLSCFUN5(INT32, SWidxinfo, SWIDXINFO, swidxinfo, INT32, 
            STRING, INT32, PSTRING, PINT32)

FCALLSCFUN5(INT, SWdefdimscale, SWDEFDIMSCALE, swdefdimscale, INT32, 
	    STRING, INT32, INT32, PVOID)
FCALLSCFUN6(INT, SWsetdimscale, SWSETDIMSCALE, swsetdimscale, INT32, 
	    STRING, STRING, INT32, INT32, PVOID)
FCALLSCFUN6(INT, SWgetdimscale, SWGETDIMSCALE, swgetdimscale, INT32, 
	    STRING, STRING, PINT32, PINT32, PVOID)
FCALLSCFUN5(INT, SWdefdimstrs, SWDEFDIMSTRS, swdefdimstrs, INT32, 
	    STRING, STRING, STRING, STRING)
FCALLSCFUN6(INT, SWsetdimstrs, SWSETDIMSTRS, swsetdimstrs, INT32, 
	    STRING, STRING, STRING, STRING, STRING)
FCALLSCFUN7(INT, SWgetdimstrs, SWGETDIMSTRS, swgetdimstrs, INT32, 
	    STRING, STRING, PSTRING, PSTRING, PSTRING,INT)
FCALLSCFUN5(INT, SWchkinfo, SWCHKINFO, swchkinfo, INT32, STRING, PINT32,
	    PINT32, INT32V)
